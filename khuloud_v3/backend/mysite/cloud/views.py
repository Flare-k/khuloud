...
class loadRecentFiles(generics.GenericAPIView):
    def get(self, request, *args, **kwargs):
        queryset = File.objects.filter(
            isFolder=False, owner=self.request.user.username).order_by('updated_at')
        print(queryset)
...
class deleteShareFolder(generics.GenericAPIView):
    ...
    def delete(self, request, id, *args, **kwargs):
        shareFolder = ShareFolder.objects.filter(
            id=id, owner=self.request.user.username)
        shareFolder.delete()
        return Response({'info': 'Database deleted'})
...
class createShareUser(generics.GenericAPIView):
    ...
    def post(self, request, id, *args, **kwargs):
        ...
        shareFolder = ShareFolder.objects.get(
            id=id, owner=self.request.user.username)
